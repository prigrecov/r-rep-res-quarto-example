---
title: "Data Article: Trier social stress test and food-choice: Behavioral, self-report & hormonal data (Data Fluency version)"
...
[Link to the paper](https://doi.org/10.1016/j.dib.2021.107245)
...
![Timeline of the experiment.](fig/Fig1.jpg){#fig-timeline}

@fig-timeline provides an exact overview of the experimental timeline for each measure we collected. All experimental sessions took place from 3 p.m. to 6 p.m. to control for circadian variations of hormonal levels. Participants were assigned to the two experimental conditions pseudo-randomly.
...
$$
\text{Amount (fruit/vegetable)} = - \frac{px}{py} Amount (sweet/salty) + \frac{m}{py}
$$ {#eq-budget}
...
The heart rate analysis is shown in @fig-heartrate.

```{r}
#| fig-width: 5
#| fig-height: 5
#| fig-cap: "Heart rate analysis"
#| label: "fig-heartrate"

source("code/03_HR_analysis.R", local = TRUE)
plot
```

(**Example 9**) add Fig 4: Cortisol and Amylase Levels in Stress and Control Groups

The cortisol and amylase levels in stress and control groups are shown in @fig-cortisol.
...
source("code/02_hormone_analysis.R", local = TRUE)
hormone_plot


```

All analyses were conducted in R in the RStudio IDE, using the packages Tidyverse [@wickham2019], stringi, BayesFactor and patchwork.
